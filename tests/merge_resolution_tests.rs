...
fn test_merge_resolution_in_range_scan() {
    ...
    match iter.next() {
        Some(Ok((key, value))) => {
            ...
            assert_eq!(value, Bytes::from("val,op1"), "Range scan failed to resolve merge");
        },
...
fn test_merge_stacking_in_range_scan() {
    ...
    match iter.next() {
        Some(Ok((key, value))) => {
            ...
            assert_eq!(value, Bytes::from("item1,item2"), "Range scan failed to resolve stacked merges");
        },
