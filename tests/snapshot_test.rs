...
fn test_snapshot_isolation_basic() {
    ...
    assert_eq!(snapshot.get(b"key1").unwrap(), Some(bytes::Bytes::from("value1")));
    assert_eq!(snapshot.get(b"key2").unwrap(), Some(bytes::Bytes::from("value2")));
    assert_eq!(snapshot.get(b"key3").unwrap(), None);
    
    // 5. Verify DB sees new state
    assert_eq!(db.get(b"key1").unwrap(), Some(bytes::Bytes::from("value1_updated")));
    assert_eq!(db.get(b"key2").unwrap(), None);
    assert_eq!(db.get(b"key3").unwrap(), Some(bytes::Bytes::from("value3_new")));
}
