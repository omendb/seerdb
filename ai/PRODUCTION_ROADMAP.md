...
- [ ] High-volume writes
  ...
  - 10M random writes
  - Measure: throughput, latency (p50/p99/p999)
- [ ] Concurrent operations
  - 8+ threads writing simultaneously
  - Mixed read/write workloads
  - Range scans during writes
- [ ] Large dataset tests
  - 100GB+ database size
...
**Acceptance Criteria**:
- Handle 10M+ operations without errors
...
- [ ] Crash during write
  - kill -9 during put()
  - Verify WAL replay recovers data
  - No data loss
- [ ] Crash during flush
  - kill -9 during memtable flush
  - Verify recovery restores state
  - WAL replay completes flush
- [ ] Crash during compaction
  - kill -9 during compact_level()
  - Verify no corruption
  - Incomplete compaction cleaned up
- [ ] Crash during recovery
  - kill -9 during WAL replay
  - Verify re-recovery works
  - No infinite loops
- [ ] Power loss simulation
  - Test with fsync disabled
  - Test with fsync enabled
  - Document durability guarantees

**Acceptance Criteria**:
- Zero data loss with SyncAll policy
- No corruption after any crash
- Recovery always completes successfully
- Documented durability guarantees per SyncPolicy
