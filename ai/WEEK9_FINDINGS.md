# Week 9: Learned Bloom Filters - Research Findings

**Date**: November 1, 2025
**Status**: ⚠️ Research complete - Not production-ready
**Key Finding**: Learned bloom filters work for specific use cases, not general-purpose storage

---

## Summary

Implemented and benchmarked learned bloom filters against traditional bloom filters. **Discovered critical limitation**: Learned blooms require data with learnable patterns and are NOT a drop-in replacement for general-purpose key-value storage.
...
**Learned Bloom Filter** (`src/bloom/learned.rs`):
...
**Benchmarks Created**:
- `examples/bloom_comparison.rs` - Compare space/FPR/speed
- `examples/bloom_debug.rs` - Diagnose model behavior
- `examples/bloom_features_test.rs` - Explain feature extraction issues
- `examples/bloom_fixed.rs` - Prove concept with proper features

**Code**: 248 lines (learned.rs) + 400+ lines (benchmarks/examples)
...
### Benchmark 2: False Positive Rate
...
Created `bloom_debug.rs` to test model on training vs unseen data:
...
Created `bloom_fixed.rs` with **proper feature extraction**:
...
- Implementation is correct (just inappropriate for use case)
- All code is research-grade and documented
- Traditional blooms remain in production use

### 📝 Documentation Debt

- Add note to `learned.rs`: "Requires patterned data, not general-purpose"
- Update PAPERS.md: Clarify learned bloom applicability
- Document findings in research section
...
**Tests**: 2 unit tests (basic functionality)
...
## Week 9 vs Paper Claims

| Claim | Paper | Our Results | Notes |
|-------|-------|-------------|-------|
| Space reduction | 90% | 73% | Confirmed at scale |
| FPR | 1% | 50% ❌ | Only with proper features |
| Works for general data | Implied | **No** | Requires patterns |
| Training cost | Not discussed | 87s for 100K | Significant overhead |

**Key Realization**: Paper evaluates on **specific dataset** (malicious URLs), not general-purpose storage.
...
- ❌ **Don't integrate learned blooms** into seerdb
- ✅ **Keep traditional blooms** for production
- ✅ **Document findings** for research purposes

### Long-term (Research)

**Workload-specific learned blooms**:
- Analyze user's key distribution
- Detect if patterns exist (URL structure, numeric IDs, etc.)
- Train custom model if beneficial
- Fallback to traditional otherwise

**Example**:
```rust
if detect_pattern(keys) {
    // Keys are URLs → use domain features
    return LearnedBloom::with_url_features(keys);
} else {
    // Arbitrary keys → use traditional
    return BloomFilter::new(keys.len(), 0.01);
}
```

### What Would Actually Help seerdb

**Skip learned blooms, focus on**:
1. **Week 10: Learned Indexes** (ALEX) - More applicable to sorted data
2. **Week 13: KV Separation** (WiscKey) - Proven 10x write amplification reduction
3. **Week 14: SIMD** - Universal speedup, no workload assumptions
...
```
<pending> - docs: Week 9 learned bloom findings (not production ready)
  - Learned bloom implementation + benchmarks
  - Root cause analysis: hash features destroy patterns
  - Proof: Fixed implementation with proper features works perfectly
  - Conclusion: Keep traditional blooms for general-purpose storage
```
...
**Next**: Move to Week 10 (Learned Indexes) or Week 13 (KV Separation) - both more applicable.

---

*Week 9 Complete - Learned bloom filters work, but not for this use case*

**Status**: Research complete, findings documented
**Production**: Stick with traditional bloom filters
**Lesson**: Context matters - paper results don't always generalize
