...
fn main() {
    ...
    let negative: Vec<String> = (10000..10000+size).map(|i| format!("key_{:04}", i)).collect();
    ...
    let test_negative: Vec<String> = (20000..20000+size).map(|i| format!("key_{:04}", i)).collect();
    ...
    let test_accuracy = ((tp + tn) as f64 / (test_positive.len() + test_negative.len()) as f64) * 100.0;
    let fpr = (fp as f64 / test_negative.len() as f64) * 100.0;
    
    println!("Results:");
    println!("  True Positives:  {} / {} ({:.1}%)", tp, test_positive.len(), (tp as f64 / test_positive.len() as f64) * 100.0);
    println!("  True Negatives:  {} / {}", tn, test_negative.len());
    println!("  False Positives: {} / {} ({:.2}% FPR)", fp, test_negative.len(), fpr);
