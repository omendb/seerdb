...
fn main() {
    ...
    println!("Benchmark 2: WiscKey vLog (value separation, threshold={}KB)", VLOG_THRESHOLD / 1024);
    println!("-------------------------------------------------");
    ...
    if write_amp_no_vlog > write_amp_with_vlog {
        ...
        println!("\nImprovement:           {:.2}x better with vLog", improvement);
...
fn run_benchmark(num_ops: usize, value_size: usize, vlog_threshold: Option<usize>) -> (f64, u64) {
    ...
    let temp_dir = format!("/tmp/seerdb_writeamp_{}", 
        vlog_threshold.map(|t| format!("vlog{}", t)).unwrap_or_else(|| "novlog".to_string()));
    ...
    println!("  Logical bytes:       {} MB ({} bytes)", 
        logical_bytes / 1_000_000, logical_bytes);
    println!("  Physical bytes:      {} MB ({} bytes)", 
        physical_bytes / 1_000_000, physical_bytes);
    ...
    println!("  Disk usage:          {} MB", stats.total_disk_bytes / 1_000_000);
