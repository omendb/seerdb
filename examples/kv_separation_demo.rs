...
fn main() {
    ...
    builder.add_with_vlog(Bytes::from("key2"), Bytes::from(large_value), &mut vlog).unwrap();
    ...
    builder.add_with_vlog(Bytes::from("key3"), Bytes::from(embedding_bytes), &mut vlog).unwrap();
    ...
    println!("File sizes:");
    println!("  SSTable: {} bytes (keys + small values + pointers)", sstable_size);
    println!("  VLog:    {} bytes (large values)", vlog_size);
    println!("  Total:   {} bytes", sstable_size + vlog_size);
    println!();

    println!("Write amplification benefit:");
    println!("  Traditional LSM: Compaction rewrites {} bytes", sstable_size + vlog_size);
    println!("  WiscKey:         Compaction rewrites {} bytes (just SSTable)", sstable_size);
    ...
    let mut sstable = seerdb::SSTable::open(&sstable_path).unwrap().with_vlog(vlog);

    println!("  key2 with vLog: OK ({} bytes)", sstable.get(b"key2").unwrap().unwrap().len());
    println!("  key3 with vLog: OK ({} bytes)", sstable.get(b"key3").unwrap().unwrap().len());
    println!();

    println!("✅ KV Separation working!");
    println!();
    println!("Benefits:");
    println!("  - Compaction only rewrites keys ({}% less data)", reduction);
