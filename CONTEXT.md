...
- Reads: 14.24µs per read (0.29x RocksDB - 71% slower)
- Writes: 10.71µs per write (0.59x RocksDB - 41% slower)
- Mixed: 19.26µs per op (0.43x RocksDB - 57% slower)
- Scans: 3.16ms per scan (0.06x RocksDB - 94% slower, but major improvement)
...
1. Range scan performance (94% slower - major improvement from 99%, but needs SSTable merging)
...
- ⚠️ Slower than RocksDB in raw performance (41-94%)
